...
public class AdminEventModifyOkAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		File event_file = multi.getFile("file");
		...
		if (event_file != null) { // 첨부파일이 존재하는 경우
			...
			// 파일을 생성
			String reFileName = "event_modify_" + ymd + "_" + fileName;
		...
		int check = dao.modifyEventBoard(dto);
		...
		if(check > 0) {
			...
			out.println("location.href='admin_event_content?no="+ board_no +"&page=" + page + "'");
			out.println("</script>");
			out.close();
		} else {
			out.println("<script>");
