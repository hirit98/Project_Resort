package com.resort.model;

public class QnaBoardDTO {
	private int qna_board_no;
	private String qna_board_writer_id;
	private String qna_board_answer_id;
	private String qna_board_type;
	private String qna_board_title;
	private String qna_board_cont;
	private String qna_board_answer;
	private String qna_board_date;
	private String qna_board_update;
	private String qna_board_file;
	private boolean qna_board_status;

	public int getQna_board_no() {
		return qna_board_no;
	}

	public void setQna_board_no(int qna_board_no) {
		this.qna_board_no = qna_board_no;
	}

	public String getQna_board_writer_id() {
		return qna_board_writer_id;
	}

	public void setQna_board_writer_id(String qna_board_writer_id) {
		this.qna_board_writer_id = qna_board_writer_id;
	}

	public String getQna_board_answer_id() {
		return qna_board_answer_id;
	}

	public void setQna_board_answer_id(String qna_board_answer_id) {
		this.qna_board_answer_id = qna_board_answer_id;
	}

	public String getQna_board_type() {
		return qna_board_type;
	}

	public void setQna_board_type(String qna_board_type) {
		this.qna_board_type = qna_board_type;
	}

	public String getQna_board_title() {
		return qna_board_title;
	}

	public void setQna_board_title(String qna_board_title) {
		this.qna_board_title = qna_board_title;
	}

	public String getQna_board_cont() {
		return qna_board_cont;
	}

	public void setQna_board_cont(String qna_board_cont) {
		this.qna_board_cont = qna_board_cont;
	}

	public String getQna_board_answer() {
		return qna_board_answer;
	}

	public void setQna_board_answer(String qna_board_answer) {
		this.qna_board_answer = qna_board_answer;
	}

	public String getQna_board_date() {
		return qna_board_date;
	}

	public void setQna_board_date(String qna_board_date) {
		this.qna_board_date = qna_board_date;
	}

	public String getQna_board_update() {
		return qna_board_update;
	}

	public void setQna_board_update(String qna_board_update) {
		this.qna_board_update = qna_board_update;
	}

	public String getQna_board_file() {
		return qna_board_file;
	}

	public void setQna_board_file(String qna_board_file) {
		this.qna_board_file = qna_board_file;
	}

	public boolean isQna_board_status() {
		return qna_board_status;
	}

	public void setQna_board_status(boolean qna_board_status) {
		this.qna_board_status = qna_board_status;
	}
}